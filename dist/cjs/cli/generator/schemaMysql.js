...
class MysqlDatabase {
    ...
    static mapTableDefinitionToType(tableDefinition, customTypes, options) {
        if (!options)
            throw new Error();
        return lodash_1.default.mapValues(tableDefinition, column => {
            switch (column.udtName) {
                ...
                default:
                    if (customTypes.indexOf(column.udtName) !== -1) {
                        column.tsType = (0, common_1.transformTypeName)(column.udtName);
                        return column;
                    }
                    else {
                        (0, debug_1.default)(`Type [${column.udtName}] has been mapped to [any] because no specific type has been found.`);
                        column.tsType = 'any';
                        return column;
                    }
            }
    ...
    static getEnumNameFromColumn(tableName, dataType, columnName) {
        return `enum_${tableName}_${columnName}`;
    }
    query(queryString) {
        return this.queryAsync(queryString);
    }
    async getEnumTypes(schema, tableName) {
        let enums = {};
        ...
        const rawEnumRecords = await this.queryAsync('SELECT table_name, column_name, column_type, data_type ' +
            'FROM information_schema.columns ' +
            whereClause, params);
        rawEnumRecords.forEach((enumItem) => {
            const enumName = MysqlDatabase.getEnumNameFromColumn(enumItem.table_name, enumItem.data_type, enumItem.column_name);
    ...
    async getTableDefinition(tableName, tableSchema) {
        ...
        const tableInfo = await this.queryAsync(`SELECT table_comment 
             FROM information_schema.tables 
             WHERE table_schema = ? AND table_name = ?`, [tableSchema, tableName]);
        // Get column information including comments
        const tableColumns = await this.queryAsync(`SELECT 
                c.column_name,
                c.data_type,
                c.is_nullable,
                c.column_key,
            ...
            WHERE c.table_name = ? and c.table_schema = ?`, [tableName, tableSchema]);
        let tableDefinition = {};
        tableColumns.forEach((schemaItem) => {
            ...
            tableDefinition[columnName] = {
                udtName: /^(enum|set)$/i.test(dataType)
                    ? MysqlDatabase.getEnumNameFromColumn(tableName, dataType, columnName)
                    : dataType,
    ...
    async getTableTypes(tableName, tableSchema, options) {
        ...
        let customTypes = lodash_1.default.keys(enumTypes);
    ...
    async queryAsync(queryString, escapedValues) {
        try {
            if (!this.db) {
                this.db = await promise_1.default.createConnection(this.connectionString);
            }
    ...
    async getPrimaryKey(schema, table) {
        const result = await this.queryAsync('SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS ' +
            'WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_KEY = "PRI"', [schema, table]);
