...
import { getDatabase, toCamelCase, transformColumnName, transformTypeName } from './common';
export async function generateAndWriteModels(db, tables, schema, options) {
...
export function buildHeader(db, tables, schema, options) {
    return `        /**
...
export async function typescriptOfTable(db, table, schema, options) {
    if (typeof db === 'string') {
        db = getDatabase(db);
    }
    let interfaces = '';
...
export function generateEnumType(enumObject, options) {
    let enumString = '';
...
export function normalizeName(name, options) {
