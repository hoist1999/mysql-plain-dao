...
export class BaseDaoUUID extends CommonDao {
    uuid_field;
    constructor(option) {
        super(option);
    ...
    async insertAsync(item) {
        ...
        const sql = format(`INSERT INTO ${this.table_name} SET ${this.uuid_field} = ?, ?`, [generatedUuid, newInsertItem]);
    ...
    async getByUuidAsync(uuid) {
        ...
        const item = await DbUtil.executeGetSingleAsync(sql, [uuid]);
        if (item) {
    ...
    async updateAsync(item) {
        ...
        const sql = format(`UPDATE ${this.table_name} SET ? WHERE ${this.uuid_field} = ?`, [updateItem, uuid]);
    ...
    async bulkInsertAsync(item_list) {
        ...
        const values_str = items_with_uuid
            .map((item) => `(${escape(item[this.uuid_field])}, ${Object.values(item)
