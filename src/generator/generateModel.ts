...
export async function typescriptOfTable(db: Database | string,
    ...
    // 检查是否需要生成 InsertModel 类型
    const tableDefinition = await (db as Database).getTableDefinition(table, schema);
    ...
    // 如果有非默认的 ID 或 UUID 字段，生成 InsertModel 类型
    if (integerIdColumn || uuidColumn) {
        ...
        if (omitFields.length > 0) {
            interfaces += `\nexport type Insert${tableName} = Omit<${tableName}, ${omitFields.join(' | ')}>;\n`;
        }
    }
