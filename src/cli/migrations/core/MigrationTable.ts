 ...
 */
async function checkMigrationsTableStructure(): Promise<{ exists: boolean; valid: boolean }> {
  try {
    ...
    const [columns] = await DbUtil.queryAsync(
    ...
    const actualColumns = columnsArray.map((c: any) => c.COLUMN_NAME);

    // Check if all required columns exist
    const hasAllColumns = expectedColumns.every(col => actualColumns.includes(col));
    
    // Check if id is primary key
    const idColumn = columnsArray.find((c: any) => c.COLUMN_NAME === 'id') as any;
    const hasPrimaryKey = idColumn?.COLUMN_KEY === 'PRI';

    // Check if name has unique constraint
    const [indexes] = await DbUtil.queryAsync(
      `SHOW INDEX FROM migrations WHERE Column_name = 'name'`
    );
    const indexesArray = Array.isArray(indexes) ? indexes : [];
    const hasUniqueName = indexesArray.some((idx: any) => idx.Non_unique === 0);
...
export async function ensureMigrationsTable(): Promise<void> {
  ...
  if (structureCheck.exists && !structureCheck.valid) {
    throw new Error(
      'Migrations table exists but has incorrect structure. ' +
...
export async function getAppliedMigrations(): Promise<MigrationRecord[]> {
  ...
  return (Array.isArray(rows) ? rows : []) as MigrationRecord[];
}
